...
def check_sleep(words):
	if 'sleep' in words or 'hibernate' in words:
		commonsense.sleepy()
		sleep()
	if ('shut' in words and 'down' in words) or 'bye' in words or 'goodbye' in words or ('code' in words and 'and' in words):
		tts_engine.say("Jarvis is shutting down")
		exit_flag = 1
		return True
def sleep():
	while True:
		try:
			mic = microphone.Microphone()
			a, s_data = mic.listen()
			stt_engine = google_stt.Google(mic)
